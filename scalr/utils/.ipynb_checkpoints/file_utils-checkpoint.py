...
def write_chunkwise_data(datapath: str,
    ...
    original_data = read_data(datapath)

    original_data.obs = original_data.obs.apply(lambda col: col.astype(
        'category').cat.add_categories('unknown').fillna("unknown"))

    if isinstance(original_data, AnnData) and feature_inds:
        raise ValueError(
            'TrainValTestSplit data for FeatureSubsetting must be AnnCollection'
        )

    if not sample_inds:
        sample_inds = list(range(len(original_data)))
